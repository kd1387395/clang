...
{ //Skill型構造体
	char name[20]; //スキル名
	int type;      //スキルの種類（0: HP回復, 1: AtkUp, 2:防御力Up, 3: 状態異常回復）
	int use_mp;    //使用MP量
	int effect;    //スキル効果
} Skill;
typedef struct 
...
enum BitState
{
	...
	Base = 0,            //     通常状態   0000 0000 0000 0000
	Poison = 1 << 0,     //(  1)毒状態     0000 0000 0000 0001
	Sleep = 1 << 1,      //(  2)睡眠状態   0000 0000 0000 0010
	Paralysis = 1 << 2,  //(  4)麻痺状態   0000 0000 0000 0100
	Burn = 1 << 3,       //(  8)火傷状態   0000 0000 0000 1000
	AtkDown = 1 << 5,    //( 32)攻撃力Down 0000 0000 0010 0000
	Dead = 1 << 6,       //( 64)死亡フラグ 0000 0000 0100 0000
	Atk_Skill = 1 << 7,  //(128)           0000 0000 1000 0000
	                     //(このフラグがONのキャラしか状態異常を付加できない）
	AtkUp = 1 << 8,      //     攻撃力Up   0000 0001 0000 0000 
	DefUp = 1 << 9,      //     防御力Up   0000 0010 0000 0000
	...
	for (i = 0; i < File_Num; i++)
	{
		if (fp = fopen(filename[i], "r"))
		{
			...
			case 0:
				...
				fscanf(fp, "%[^,],%d,%d,%d,%u,%d,%d", c->sp.name, &c->sp.hp, &c->sp.atk, &c->sp.def, &c->sp.state, &c->sp.maxhp, &c->sp.mp);
				break;
			case 1:
				for (int j = 0; j < Skil_Num; j++)
				{
					//name,type,use_mp,effect
					fscanf(fp, "%[^,],%d,%d,%d", c->skl[j].name, &c->skl[j].type, &c->skl[j].use_mp, &c->skl[j].effect);
				}
				break;
			case 2:
				for (int j = 0; j < Mob_Num; j++)
				{
					...
					fscanf(fp, "%[^,],%d,%d,%d,%u,%d\n", (m + j)->sp.name, &(m + j)->sp.hp, &(m + j)->sp.atk, &(m + j)->sp.def, &(m + j)->sp.state, &(m + j)->sp.rate);
				}
				break;
			}
			fclose(fp);
...
void ClearFlag(UINT* s)
{
	...
	{
		printf("どの状態を解除しますか?\n");
		printf("1:毒 2:睡眠 3:麻痺 4:火傷 5:攻撃↑ 6:攻撃↓ 7:状態異常全回復 0:入力終了\n>");
		scanf("%d", &a);
		if (a == 0)
		{
			break;
